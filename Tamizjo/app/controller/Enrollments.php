...
/*
            //Validar inputs del enrollment
            if (empty($data['id_student'])) {
                $data['idError'] = 'Id del estudiante';
            } else if (!$this->userModel->findStudentById($data['id_student'])) {
                $data['idError'] = 'Estudiante no existe';
            }

            if (empty($data['id_course'])) {
                $data['id_courseError'] = 'Id del curso';
            } else if (!$this->courseModel->findCourseById($data['id_course'])) {
                $data['id_courseError'] = 'Curso no existe';
            }
*/
...
/*
            //Validar inputs student
            if (empty($data['id_student'])) {
                $data['idError'] = 'Id del estudiante';
            } else if (!$this->userModel->findStudentById($data['id_student'])) {
                $data['idError'] = 'Estudiente no existe';
            }
            if (empty($data['id_course'])) {
                $data['id_courseError'] = 'Id del curso';
            } else if (!$this->courseModel->findCourseById($data['id_course'])) {
